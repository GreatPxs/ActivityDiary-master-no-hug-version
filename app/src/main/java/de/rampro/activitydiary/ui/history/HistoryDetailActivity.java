...
public class HistoryDetailActivity extends BaseActivity implements LoaderManager.LoaderCallbacks<Cursor>{
            ...
            if ((cursor != null)) {
                if(token == READ_ALL){
                    if(cursor.moveToFirst()) {
                        ...
                        @SuppressLint("Range") long endMillis = cursor.getLong(cursor.getColumnIndex(ActivityDiaryContract.Diary.END));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        int itemId = item.getItemId();//            case R.id.action_edit_delete:
        /* TODO: DELETE diary entry */
//                finish();
//                break;
        if (itemId == android.R.id.home) {/* cancel edit */
            finish();
        } else if (itemId == R.id.action_edit_done) {/* finish edit and save */
            if (checkConstraints()) {
